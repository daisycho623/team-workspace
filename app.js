...
const WORKER_KEY='cx-worker-settings-v1';
function readWorkerSettings(){try{const value=JSON.parse(localStorage.getItem(WORKER_KEY));return Array.isArray(value)&&value.every(n=>typeof n==='string'&&n.trim())?value:null}catch{return null}}
let workers=readWorkerSettings()||[...(window.WORKERS||[])];
const HIDDEN_WORKER_KEY='cx-hidden-workers-v1';
let hiddenWorkers=new Set();
try{const names=JSON.parse(localStorage.getItem(HIDDEN_WORKER_KEY));if(Array.isArray(names))hiddenWorkers=new Set(names.filter(n=>typeof n==='string'))}catch{}
function visibleWorkers(){return workers.filter(name=>!hiddenWorkers.has(name))}
...
function renderWorkerAdmin(){
 const box=$('#workerAdminList');if(!box)return;
 box.innerHTML=visibleWorkers().map(name=>`<label class="worker-admin-item"><span><input type="checkbox" class="worker-check" value="${esc(name)}" aria-label="${esc(name)} 숨김 선택"> ${esc(name)}</span></label>`).join('');
}
function addWorker(){
 const name=$('#workerName').value.trim();
 if(!name){alert('추가할 작업자 이름을 입력해 주세요.');return}
 if(workers.includes(name)&&!hiddenWorkers.has(name)){alert('이미 등록된 작업자입니다.');return}
 const next=workers.includes(name)?[...workers]:[...workers,name],hidden=new Set(hiddenWorkers);hidden.delete(name);
 if(!persistWorkers(next,hidden))return;
 $('#workerName').value='';renderWorkerAdmin();filters();render();
}
function hideWorkers(names){
 if(!names.length){alert('숨길 작업자를 선택해 주세요.');return}
 const hidden=new Set(hiddenWorkers);names.filter(name=>workers.includes(name)).forEach(name=>hidden.add(name));
 if(!persistWorkers(workers,hidden))return;
 renderWorkerAdmin();filters();render();
}
function persistWorkers(names,hidden){
 try{localStorage.setItem(WORKER_KEY,JSON.stringify(names));localStorage.setItem(HIDDEN_WORKER_KEY,JSON.stringify([...hidden]))}catch{alert('이 브라우저에 작업자 설정을 저장할 수 없습니다.');return false}
 workers=names;hiddenWorkers=hidden;$('#saveStatus').textContent='● 작업자 목록 설정 저장됨';return true;
}
let loading=false;
async function requestServer(payload){
  if(!window.APPS_SCRIPT_URL)throw new Error('config.js에 Apps Script 웹 앱 URL을 설정해 주세요.');
  const response=await fetch(window.APPS_SCRIPT_URL,{method:'POST',cache:'no-store',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify(payload),signal:AbortSignal.timeout(30000)});
...
async function load(){
  ...
  try{
    ...
    workers=[...new Set([...(readWorkerSettings()||window.WORKERS||[]),...(Array.isArray(result.workers)?result.workers:[]),...data.map(r=>r[2])].filter(Boolean))];
    ...
    $('#saveStatus').textContent='● 조회 실패: '+error.message;
...
async function saveAll(){
  ...
  const tasks=orderedRows.map(row=>[0,1,2,3,4,5,6,9,10].map(c=>c===0&&pendingRows.has(row)?registrationLabel(row[0]):row[c]??''));
  try{
    const result=await requestServer({action:'save',tasks});
    data=orderedRows.map((row,i)=>[tasks[i][0],...row.slice(1)]);newRows.clear();edits={};try{localStorage.removeItem(KEY)}catch{}
...
function selected(){let q=$('#search').value.toLowerCase(),w=$('#worker').value,s=$('#status').value;return data.map((r,i)=>({r,i})).filter(x=>inSelectedMonth(x.r[0])&&(currentTab!=='active'||!['완료','보류','이월'].includes(x.r[3]))&&(!q||x.r.join(' ').toLowerCase().includes(q))&&(w==='all'||x.r[2]===w)&&(s==='all'||x.r[3]===s)).sort((a,b)=>Number(newRows.has(b.i))-Number(newRows.has(a.i))||(newRows.has(a.i)?b.i-a.i:a.i-b.i))}
...
function people(rows){$('#people').innerHTML=workers.map(n=>{let own=rows.filter(r=>r[2]===n);if(!own.length)return '';let total=own.reduce((s,r)=>s+(+r[9]||0),0),months={};own.forEach(r=>(months[monthOf(r[0])]??=[]).push(r));let groups=Object.entries(months).map(([m,list])=>`<div class="person-month"><div class="month-row">${m}</div>${list.map(r=>`<div class="person-task"><span>${esc(registrationLabel(r[0]))}</span><span class="rms">${esc(r[1])}</span><span><b class="mini-status ${statusClass(r[3])}">${esc(r[3])}</b></span><span>${esc(r[4])}</span><strong>${esc(r[5])}</strong><span class="person-notes">${esc(r[6])}</span><span>${esc(r[9]??'')}</span><span>${esc(r[10]??'')}</span></div>`).join('')}</div>`).join('');return `<section class="person-section"><div class="person-header"><b>${esc(n)}</b><strong>${total}</strong></div>${groups}</section>`}).join('')}
function deleteSelected(){let ids=$$('.row-check:checked').map(x=>+x.dataset.check);if(!ids.length){alert('삭제할 업무를 선택해 주세요.');return}if(!confirm(`${ids.length}개의 업무를 목록에서 제외하시겠습니까? 변경사항 저장 시 서버에 반영됩니다.`))return;const pending=new Set([...newRows].map(i=>data[i]));ids.sort((a,b)=>b-a).forEach(i=>data.splice(i,1));newRows=new Set(data.flatMap((r,i)=>pending.has(r)?[i]:[]));deleteMode=false;$('#deleteToggle').textContent='삭제';$('#saveStatus').textContent='● 삭제를 반영하려면 변경사항 저장을 눌러 주세요';render()}
...
function showView(view){
 ...
 $('.month-control').hidden=view==='workers';$('.tools').hidden=false;$('.tools > span').hidden=view==='workers';$('#editHint').hidden=view!=='list';$('#newTask').hidden=view!=='list';
...
$('#workerHide').onclick=()=>hideWorkers($$('.worker-check:checked').map(input=>input.value));
$('#workerName').onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();addWorker()}};
